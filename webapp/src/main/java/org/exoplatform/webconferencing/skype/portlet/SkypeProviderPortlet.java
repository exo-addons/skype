...
public class SkypeProviderPortlet extends GenericPortlet {
  ...
  public void init() throws PortletException {
    ...
    try {
      this.provider = (SkypeProvider) webConferencing.getProvider(SkypeProvider.SKYPE_TYPE);
    } catch (ClassCastException e) {
      LOG.error("Provider " + SkypeProvider.SKYPE_TYPE + " isn't an instance of " + SkypeProvider.class.getName(), e);
    }
  }
    ...
    if (this.provider != null) {
      try {
        ...
        js.require("SHARED/webConferencing", "webConferencing")
          .require("SHARED/webConferencing_skype", "skypeProvider")
          .addScripts("if (skypeProvider) { skypeProvider.configure(" + settingsJson
              + "); webConferencing.addProvider(skypeProvider); webConferencing.update(); }");
      } catch (Exception e) {
